...
class TransformerEncoder_cross(nn.Module):
    ...
    def forward(self, src: Tensor, query: Tensor) -> Tensor:
        output = src
        for mod in self.layers:
            output = mod(output, query)
        return output
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward, dropout=0., batch_first=False) -> None:
...
class TransformerEncoderLayer_cross(nn.Module):
    ...
    def forward(self, src: Tensor, query: Tensor) -> Tensor:
        src2 = self.self_attn(query, src, src)[0]
        src = src + src2
        src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))
        src = src + src2
        return src
