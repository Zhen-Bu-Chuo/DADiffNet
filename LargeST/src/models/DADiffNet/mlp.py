...
class FusionMLP(nn.Module):
    def __init__(self, input_dimz, hidden_dim, out_dim, graph_num, first, **model_args):
        ...
        self.if_augment = model_args["if_augment"] or model_args["if_backward"]
        ...
        if self.graph_num > 1:
            ...
            if self.if_augment:
                self.fusion_augment_linear = nn.Linear(in_features=self.hidden_dim, out_features=self.fusion_dim,
                                                        bias=True)

            self.fusion_model = nn.Sequential(
                *[MultiLayerPerceptron(input_dimz=self.graph_num * self.fusion_dim,
        ...
        if not self.first:
            if self.if_feedback:
                ...
                if self.if_augment:
                    self.augment_att = nn.MultiheadAttention(embed_dim=self.fusion_dim,
                                                              num_heads=self.nhead,
                                                              batch_first=True)
                    self.augment_fc = nn.Sequential(
                        nn.Linear(in_features=self.fusion_dim, out_features=self.node_dim, bias=True),
                        nn.Sigmoid()
                    )

    def forward(self, history_data, time_series_emb, predict_emb,
                node_forward_emb, node_augment_emb, hidden_forward_emb, hidden_augment_emb):
        tem_emb = []
        if self.if_time_in_day:
        ...
        if not self.first:
            if self.if_feedback:
                ...
                if self.if_augment:
                    node_augment_emb = node_augment_emb[0]
                    hidden_augment_emb = hidden_augment_emb[0]
                    hidden_augment_emb = \
                        self.augment_att(hidden_augment_emb, hidden_augment_emb, hidden_augment_emb)[0]
                    hidden_augment_emb = self.augment_fc(hidden_augment_emb)
                    node_augment_emb = [node_augment_emb * hidden_augment_emb]
        ...
        if self.graph_num > 1:
            ...
            if self.if_augment:
                augment_emb = torch.cat(time_series_emb + predict_emb + node_augment_emb + tem_emb, dim=2)
                hidden_augment = self.fusion_graph_model(augment_emb)
                hidden_augment = [self.fusion_augment_linear(hidden_augment)]

            hidden = torch.cat(hidden_forward + hidden_augment, dim=2)
            predict = self.fusion_model(hidden)
            return predict, hidden_forward, hidden_augment, node_forward_emb, node_augment_emb
        else:
            hidden = torch.cat(
                time_series_emb + predict_emb + node_forward_emb + node_augment_emb + tem_emb, dim=2)
            hidden = self.fusion_model(hidden)
            predict = self.fusion_linear(hidden)
        return predict, [hidden], [hidden], [hidden], node_forward_emb, node_augment_emb
